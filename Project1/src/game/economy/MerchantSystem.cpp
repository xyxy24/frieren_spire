    ...
    const run::PlayerProgress& player, const run::Seed seed, const std::size_t itemCount)
{
    ...
    {
        if (item.id == 0U || item.price < 0) throw std::invalid_argument("merchant catalog contains invalid item");
    ...
    if (eligible.size() < itemCount) throw std::invalid_argument("merchant catalog has insufficient eligible stock");
