...
- `EnemyStateView` 还公开锁定后的朝向。SFML 表现层可按敌人类型和 `windingUp/attackActive` 选择贴图并水平翻转，但贴图尺寸、透明区域和视觉偏移不得反向改变领域碰撞箱。当前无头骑士使用 `64×64` 的 idle/windup/attack PNG，脚底中心对齐其 `42×58` 碰撞箱；资源由 CMake 复制到可执行文件旁的 `assets/`。
