...
基础攻击、三个魔法槽与敌人攻击均通过该入口。Resolver 按伤害来源分别记录最后结算序列，相同来源和序列不会重复扣血，不同魔法槽之间保持隔离。遗物系统后续只能在明确的来源修正、目标修正或结果事件阶段挂接，不得绕过 Resolver 直接扣除战斗 HP。
...
- 本局实例状态通过稳定 ID 引用定义。
- `SpellSystem` 从三个已装备内容 ID 建立每场战斗的槽位运行状态；定义保存伤害、冷却与范围，槽位实例单独保存剩余冷却。`CombatSession` 请求携带装备快照并统一应用命中后的 HP 变化，表现层只读取 `SpellSlotView` 绘制冷却。
