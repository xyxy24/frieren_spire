...
本文定义玩法逻辑架构与 SFML 平台边界，用于指导后续 C++20 实现。项目已选择 SFML 3.1.0 与 CMake，当前已经存在窗口/输入适配、纯 C++ 玩家控制器和 CTest 测试目标；音频、内容数据格式和完整测试框架仍需后续决定。文中的未创建目录仍属于计划边界。
...
楼层类型由有状态 `FloorScheduler` 调度：Boss 楼层编号由调用方显式配置，不在调度器内固定每幕层数；普通候选层使用确定性随机，并通过可配置最大间隔为商店和事件提供保底。Boss 层冻结保底计数，两个保底同时临近时提前安排更紧迫者。
